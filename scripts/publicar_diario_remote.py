#!/usr/bin/env python3
"""
publicar_diario_remote.py — Versão sem BigQuery client para uso em agentes remotos.
Aceita dados pré-buscados via JSON files, gera HTMLs e publica no GitHub Pages.

Uso:
  GITHUB_TOKEN=xxx python3 publicar_diario_remote.py --data-dir /tmp/csops --data-ref 2026-05-14
"""
...
# ─── Process rows from BQ MCP JSON ────────────────────────────────────────────

def process_efetividade(rows):
    result = []
    for row in rows:
        n_atv        = row.get("total_atividades") or 0
        n_ef         = row.get("total_efetivas") or 0
        esc_atv      = row.get("escolas_com_atividade") or 0
        esc_ef       = row.get("escolas_com_ativ_efetiva") or 0
        prazo        = row.get("escolas_com_prazo") or 0
        abord        = row.get("abordadas_prazo") or 0
        esc_ef_prazo = row.get("escolas_efetivas_prazo") or 0
        tasks        = row.get("total_tasks") or 0
        conc         = row.get("total_tasks_conc") or 0
        cart         = row.get("carteira") or 0
        email        = row.get("email", "")
...
def process_esforco(rows):
    result = {}
    for row in rows:
        result[row.get("email", "")] = {
            "deals_tocados": row.get("deals_tocados") or 0,
            "total_atv": row.get("total_atv") or 0,
            "n_whatsapp": row.get("n_whatsapp") or 0,
            "n_chamada": row.get("n_chamada") or 0,
            "n_reuniao": row.get("n_reuniao") or 0,
            "n_email": row.get("n_email") or 0,
            "n_neg": row.get("n_neg") or 0,
            "n_pos": row.get("n_pos") or 0,
        }
    return result

def process_canal_mix(rows):
    out = {"efetivos": {}, "nao_efetivos": {}}
    for row in rows:
        key = "efetivos" if row.get("virou") else "nao_efetivos"
        total = ((row.get("n_whatsapp") or 0) + (row.get("n_chamada") or 0)
                 + (row.get("n_reuniao") or 0) + (row.get("n_email") or 0))
        def pct(n): return round(n / total * 100) if total > 0 else 0
        out[key] = {
            "n_deals": row.get("n_deals") or 0, "total": total,
            "n_whatsapp": row.get("n_whatsapp") or 0, "pct_whatsapp": pct(row.get("n_whatsapp") or 0),
            "n_chamada":  row.get("n_chamada")  or 0, "pct_chamada":  pct(row.get("n_chamada")  or 0),
            "n_reuniao":  row.get("n_reuniao")  or 0, "pct_reuniao":  pct(row.get("n_reuniao")  or 0),
            "n_email":    row.get("n_email")    or 0, "pct_email":    pct(row.get("n_email")    or 0),
        }
    return out

def process_totais(rows):
    row = rows[0] if rows else {}
    elegiveis = row.get("elegiveis") or 0
    efetivas  = row.get("efetivas_prazo") or 0
...
def diagnostico(ef_row, esf):
    ...
    if cart > 0:
        ...
        if pct_cob < 20:
            parts.append(f"cobertura crítica: {dt}/{cart} deals tocados ({pct_cob}%)")
        else:
            parts.append(f"cobertura: {dt}/{cart} deals tocados")
    return ". ".join(parts)
...
def gerar_html_esforco(dados_ef, dados_esf, canal_mix, data_ref, totais=None):
    ref_str = data_ref.strftime("%d/%m/%Y")
    ...
    for r in dados_ef:
        ...
        consultants.append({**r, "esf": esf, "meta70": meta70, "conv_rec": conv_rec,
                            "diagnostico": diagnostico(r, esf)})
    ...
    total_prazo_time = sum(c["prazo"] for c in consultants)
    total_ef_time    = sum(c["ef"]    for c in consultants)
    pef_time = totais["pef"] if totais else (round(total_ef_time / total_prazo_time * 100, 1) if total_prazo_time > 0 else 0)
...
<div class="kpi-row">
  <div class="kpi {'green' if pef_time >= 70 else 'amber' if pef_time >= 60 else 'blue'}">
    <div class="kpi-lbl">Efetividade agregada</div>
    <div class="kpi-val">{pef_time}%</div>
    <div class="kpi-sub">meta: 70%</div>
  </div>
  <div class="kpi blue">
    <div class="kpi-lbl">Conversões faltando</div>
    <div class="kpi-val">{total_conv_rec}</div>
    <div class="kpi-sub">para o time atingir 70%</div>
  </div>
  <div class="kpi blue">
    <div class="kpi-lbl">Consultores abaixo de 60%</div>
    <div class="kpi-val">{len(low)}</div>
    <div class="kpi-sub">de {len(consultants)} com dados</div>
  </div>
  <div class="kpi green">
    <div class="kpi-lbl">Consultores acima de 70%</div>
    <div class="kpi-val">{len(high)}</div>
    <div class="kpi-sub">já na meta</div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="card-body">
    <p style="font-size:13px;color:#333;margin-bottom:16px;line-height:1.65">
      O time está com efetividade agregada de <strong>{pef_time}%</strong>.
    ...
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          <th>Consultor (foco)</th><th style="text-align:right">Carteira</th>
          <th style="text-align:right">Efetivas hoje</th><th style="text-align:right">% atual</th>
          <th style="text-align:right">Para 70%</th><th style="text-align:right">Conv. a recuperar</th>
  ...
  <div class="card-body">
    ...
    <div class="canal-grid">
      <div class="canal-card">
        ...
        <div class="canal-card-sub">{ef_data.get('n_deals',0)} deals com pelo menos 1 atividade efetiva.</div>
        {_canal_bars(ef_data, "ef")}
      </div>
      <div class="canal-card">
        <div class="canal-card-title">Deals tocados que NÃO viraram</div>
        <div class="canal-card-sub">{nef_data.get('n_deals',0)} deals com atividade mas sem efetiva.</div>
...
<div class="card">
  <div class="card-head"><span class="card-title">Bloco A — Recuperação de gap <span style="font-size:11px;font-weight:500;color:var(--red);margin-left:6px">prioridade máxima</span></span></div>
  <div class="card-body">
    <div class="tbl-wrap">
      <table>
        <thead><tr><th>Consultor</th><th style="text-align:right">Meta hoje</th><th>Ação específica</th></tr></thead>
...
# ─── HTML: Meta Distribuída ───────────────────────────────────────────────────

def gerar_html_meta_distribuida(dados_ef, data_ref, totais=None):
    ref_str = data_ref.strftime("%d/%m/%Y")
    consultants = sorted([r for r in dados_ef if r["cart"] > 0], key=lambda x: x["pef"], reverse=True)
    if not consultants:
        return ""
    ...
    for r in rows:
        ...
        if r["falta"] == 0 and r["meta_dia"] > 0:
            status  = "✅ Meta batida"; s_style = "color:#15803d;font-weight:600"
        elif r["nao_ef"] == 0:
            status  = "✅ Carteira 100%"; s_style = "color:#15803d;font-weight:600"
        else:
            status  = f"+{r['falta']} para meta"; s_style = "color:#991b1b;font-weight:600"
        table_rows += (
            f'<tr><td class="nome">{r["nome"]}{seg_html}</td>'
            f'<td class="num">{r["cart"]}</td><td class="num">{r["nao_ef"]}</td>'
            f'<td class="num">{r["ef_hoje"]}</td><td class="num">{r["meta_dia"]}</td>'
            f'<td class="num"><div class="bar-wrap"><div class="bar-bg">'
            f'<div class="bar-fill" style="width:{min(pct,100):.0f}%;background:{bar_color}"></div></div>'
            f'<span class="{ef_cls}">{pct:.1f}%</span></div></td>'
            f'<td style="{s_style}">{status}</td></tr>\n'
        )
...
<div style="margin-bottom:20px">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">
    ...
    <div class="kpi amber"><div class="kpi-lbl">Em andamento</div><div class="kpi-val">{em_andamento}</div><div class="kpi-sub">efetivaram mas abaixo de 50%</div></div>
    <div class="kpi blue"><div class="kpi-lbl">Sem efetivação hoje</div><div class="kpi-val">{sem_ef_hoje}</div><div class="kpi-sub">precisam de atenção</div></div>
  </div>
</div>
<div class="card">
  <div class="card-head"><span class="card-title">Análise do dia</span></div>
  <div class="card-body" style="padding:0">
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          <th>Consultor</th><th style="text-align:right">Carteira</th><th style="text-align:right">Não efetivas</th>
          <th style="text-align:right">Efetivadas hoje</th><th style="text-align:right">Meta do dia</th>
          <th>% Ef. hoje</th><th>Status</th>
        </tr></thead>
...
def gerar_diretivo(dados_ef, canal_mix, data_ref, totais=None):
    ...
    if canal_pct > 0:
        linhas.append(f"• Canal que mais converte: *{canal_top}* ({canal_pct}% das atividades efetivas).")
    return "\n".join(linhas)
...
def _adicionar_links_portal(token, nome_ef, nome_esf, data_ref):
    ...
    if html_novo == html_atual:
        print("  portal já contém esses links")
        return

    data = {"message": f"Adiciona links {data_ref} ao portal",
            "content": base64.b64encode(html_novo.encode("utf-8")).decode(), "sha": sha}
...
def main():
    ...
    if not token:
        sys.exit("❌  GITHUB_TOKEN não configurado.")
    ...
    print(f"▶ Processando dados para {data_ref.strftime('%d/%m/%Y')}...")

    rows_hoje  = load("efetividade_hoje.json")
    rows_ontem = load("efetividade_ontem.json")
    rows_esf   = load("esforco.json")
    rows_canal = load("canal_mix.json")
    rows_tot   = load("totais.json")

    dados_hoje  = process_efetividade(rows_hoje)
    dados_ontem = process_efetividade(rows_ontem)
    dados_esf   = process_esforco(rows_esf)
    canal_mix   = process_canal_mix(rows_canal)
    totais      = process_totais(rows_tot)

    if not dados_hoje:
        sys.exit(f"❌  Nenhum dado de efetividade para {data_ref}.")
    ...
    html_esf  = gerar_html_esforco(dados_hoje, dados_esf, canal_mix, data_ref, totais)
    html_meta = gerar_html_meta_distribuida(dados_hoje, data_ref, totais)

    nome_ef  = f"efetividade_{sufixo}.html"
    nome_esf = f"analise_de_esforco_time.html"

    print("  publicando arquivos fixos no GitHub Pages...")
    ...
    print("  atualizando portal index.html...")
    ...
    print("  disparando mensagem no GChat...")
    send_gchat(msg)
